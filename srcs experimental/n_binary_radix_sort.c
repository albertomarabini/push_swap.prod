...
/*   Updated: 2023/11/07 04:33:16 by amarabin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	n_binary_radix_sort(t_stacks *stks, t_n_base *n_base, int size)
{
	...
	// int last_digit_treshold = ipow(n_base->base, n_base->dgt_count - 1) - 1;
	// total_moves += r(stks->a, 'a');
	// while (head(stks->a,0) != last_digit_treshold){
	// 	if(head(stks->a,0) < last_digit_treshold)
	// 		total_moves += r(stks->a, 'a');
	// 	else
	// 		total_moves += p(stks->b, stks->a, 'b');
	// }
